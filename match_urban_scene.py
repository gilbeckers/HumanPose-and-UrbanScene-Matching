...
if __name__ == '__main__':
    ...
    detector, matcher = feature_operations.init_feature(feature_name)
    ...
    (mask, p_model_good, p_input_good, H, H2) = feature_operations.match_and_draw('find_obj', matcher, desc_model,
                                                                              desc_input, kp_model, kp_input,
                                                                              model_image, input_image)

    # --------- STEP 3: VALIDATE HOMOGRAPHY/PERSPECTIVE MATRIX ----------------------
    if(feature_operations.validate_homography(H)):  # H = perspective transformation matrix
    ...
    (p_persp_trans_input, persp_trans_input_img ) = feature_operations.perspective_correction(H2, p_model_good, p_input_good, model_image, input_image)
    ...
    feature_operations.affine_trans_interaction(p_model_good, p_input_good, model_image, input_image, "")



    '''--------- STEP 5: INTERACTION BETWEEN HUMAN AND URBAN SCENE WiITH perspective correction------------------ '''
    feature_operations.affine_trans_interaction(p_model_good, p_persp_trans_input, model_image, persp_trans_input_img, "incl persp corr")
