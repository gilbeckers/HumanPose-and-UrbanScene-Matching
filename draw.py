...
def draw_humans(npimg, humans, imgcopy=False):
    ...
    if type(humans) is list:
        ...
        return npimg
    else:  # (no list, but plain np.array)
        for person in range(0,amount_people):
            # draw point
            for i in range(CocoPart.Background.value):
                j = i + (person*18)
                ...
                if body_part[0] == 0 and body_part[1] == 0 or body_part[0] == np.nan or body_part[1] == np.nan or body_part[0] <0 or body_part[1] <0 or math.isnan(body_part[0]) or math.isnan(body_part[1]):
                    center = (0, 0)
                    centers[i] = center
                    continue
            ...
            for pair_order, pair in enumerate(CocoPairsRender):
                ...
                # npimg = cv2.line(npimg, centers[pair[0]], centers[pair[1]], common.CocoColors[pair_order], 3)
                if centers[pair[0]] != (0,0) and  centers[pair[1]]!=(0,0):
